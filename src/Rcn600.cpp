...
#endif // __ENABLE_FAST_DIGITAL_PIN__	
}
...
#ifdef __ENABLE_FAST_DIGITAL_PIN__
	bitWrite(_Buffer[_messageCounter].Bytes[_bitCounter / 8], (_bitCounter % 8), _DATA_pin->digitalReadFast());
#else
	bitWrite(_Buffer[_messageCounter].Bytes[_bitCounter / 8], (_bitCounter % 8), digitalRead(_DATA_pin));
#endif
...
void Rcn600::ISR_SUSI(void) {
	if (!_Buffer[_messageCounter].Complete) {
		if (millis() - _lastByte_time > SYNC_TIME) {	//se sono passati piu' di 9ms dall'ultimo Byte ricevuto, devo resettare la lettura dei dati
			...
			if ((_bitCounter % 8) == 0) {
				if (_bitCounter == 16) {	// letti 2 byte, controllo se il messaggio NON serve per manipolare le CVs
					if (! ( (_Buffer[_messageCounter].Bytes[0] == 119) || (_Buffer[_messageCounter].Bytes[0] == 123) || (_Buffer[_messageCounter].Bytes[0] == 127) ) ) {
						_Buffer[_messageCounter].Complete = true;
						_bitCounter = 0;
						++_messageCounter;

						if (_messageCounter > BUFFER_LENGTH) {
							_messageCounter = 0;
						}
					}
					/*
					* else {
					*	non faccio nulla perchè devo acquisire il terzo Byte
					* }
					*/
				}
				else if (_bitCounter == 24) {	//messaggio da 3 Byte acquisito
					_bitCounter = 0;

					noInterrupts();
					processCvMessage(_Buffer[_messageCounter]);
					interrupts();
				}
			}
		}
...
void Rcn600::processCvMessage(Rcn600_Message message) {
	...
	case 119: {
		...
		*/
		...
		CV_Number = 897 + (message.Bytes[1] - 128);
	...
	case 123: {
		...
		*/
		...
		if (isCVvalid(CV_Number)) {
			...
			operation = bitRead(message.Bytes[2], 4);																							// leggo quale operazione sui bit e' richiesta
			bitValue = bitRead(message.Bytes[2], 3);																							// leggo il valore del bit da confrontare/scrivere
			bitPosition = ((bitRead(message.Bytes[2], 0) * 1) + (bitRead(message.Bytes[2], 1) * 2) + (bitRead(message.Bytes[2], 2) * 4));		// leggo in quale posizione si trova il bit su cui fare il confronto/scrittura
			...
			case 1: {	//scrittura del bit
				if (!((CV_Number == 900) || (CV_Number == 901) || (CV_Number == 940) || (CV_Number == 941) || (CV_Number == 980) || (CV_Number == 981))) {
					if (notifySusiCVWrite) {
						bitWrite(CV_Value, bitPosition, bitRead(message.Bytes[2], 3));	//scrivo il nuovo valore del bit
						if (notifySusiCVWrite((897 + (message.Bytes[1] - 128)), CV_Value) == CV_Value) {	//memorizzo il nuovo valore della CV
							Data_ACK();
						}
					}
		...
		*/
		...
		if (isCVvalid(CV_Number)) {
			...
			if (!((CV_Number == 901) || (CV_Number == 941) || (CV_Number == 981))) {
				if (((CV_Number == 900) || (CV_Number == 940) || (CV_Number == 980)) && (message.Bytes[2] == 8)) {	//si vuole eseguire un Reset delle CVs
					...
					if (notifySusiCVWrite) {
						if (notifySusiCVWrite(CV_Number, message.Bytes[2]) == message.Bytes[2]) {
							Data_ACK();
						}
					}
					/* nel caso in cui non e' implementato un sistema di memorizzazione CVs, non faccio nulla
...
void Rcn600::process(void) {
	for (uint8_t i = 0; i < BUFFER_LENGTH; ++i) {
		if (_Buffer[i].Complete) {		//controllo che sia stato ricevuto un messaggio completo
			/* Devo controllare il valore del primo Byte */

#ifdef NOTIFY_RAW_MESSAGE
			if (notifySusiRawMessage) {
				notifySusiRawMessage(_Buffer[i]);
			}
#endif

			switch (_Buffer[i].Bytes[0]) {
			case 96: {
				/* "Funktionsgruppe 1" : 0110-0000 (0x60 = 96) 0 0 0 F0 - F4 F3 F2 F1 */
				if (notifySusiFunc) {
					notifySusiFunc(SUSI_FN_0_4, _Buffer[i].Bytes[1]);
				}
				break;
			}
			case 97: {
				...
				if (notifySusiFunc) {
					notifySusiFunc(SUSI_FN_5_12, _Buffer[i].Bytes[1]);
				}
				break;
			}
			case 98: {
				...
				if (notifySusiFunc) {
					notifySusiFunc(SUSI_FN_13_20, _Buffer[i].Bytes[1]);
				}
				break;
			}
			case 99: {
				...
				if (notifySusiFunc) {
					notifySusiFunc(SUSI_FN_21_28, _Buffer[i].Bytes[1]);
				}
				break;
			}
			case 100: {
				...
				if (notifySusiFunc) {
					notifySusiFunc(SUSI_FN_29_36, _Buffer[i].Bytes[1]);
				}
				break;
			}
			case 101: {
				...
				if (notifySusiFunc) {
					notifySusiFunc(SUSI_FN_37_44, _Buffer[i].Bytes[1]);
				}
				break;
			}
			case 102: {
				...
				if (notifySusiFunc) {
					notifySusiFunc(SUSI_FN_45_52, _Buffer[i].Bytes[1]);
				}
				break;
			}
			case 103: {
				...
				if (notifySusiFunc) {
					notifySusiFunc(SUSI_FN_53_60, _Buffer[i].Bytes[1]);
				}
				break;
			}
			case 104: {
				...
				if (notifySusiFunc) {
					notifySusiFunc(SUSI_FN_61_68, _Buffer[i].Bytes[1]);
				}
				break;
			}
			case 109: {
				...
				*/
				...
				funcState = bitRead(_Buffer[i].Bytes[1], 7);	// leggo il valore dello stato 'D'

				bitWrite(_Buffer[i].Bytes[1], 7, 0);			// elimino il valore dello stato

				functionNumber = _Buffer[i].Bytes[1];			// i restanti bit identificano la Funzione 'L'
			...
			case 110: {	// && 111
				...
				*/
				...
				if (_Buffer[i+1].Bytes[0] == 111) {					// Posso eseguire il comando solo se ho ricevuto sia il Byte piu' significativo che quello meno significativo
					if (notifySusiBinaryState) {				// Controllo se e' presente il metodo per gestire il comando
						static uint16_t Command;
						static uint8_t State;

						Command = _Buffer[i + 1].Bytes[1];				// memorizzo i bit "piu' significativ"
						Command = Command << 7;					// sposto i bit 7 posti a 'sinistra'
						Command |= _Buffer[i].Bytes[1];				// aggiungo i 7 bit "meno significativi"

						State = bitRead(_Buffer[i].Bytes[1], 7);

						notifySusiBinaryState(Command, State);
					}

					_Buffer[i + 1].Complete = false;
				}
				break;
			}
			case 64: {
				...
				*/
				if (notifySusiAux) {
					notifySusiAux(SUSI_AUX_1_8, _Buffer[i].Bytes[1]);
				}
				break;
			}
			case 65: {
				...
				if (notifySusiAux) {
					notifySusiAux(SUSI_AUX_9_16, _Buffer[i].Bytes[1]);
				}
				break;
			}
			case 66: {
				...
				if (notifySusiAux) {
					notifySusiAux(SUSI_AUX_17_24, _Buffer[i].Bytes[1]);
				}
				break;
			}
			case 67: {
				...
				if (notifySusiAux) {
					notifySusiAux(SUSI_AUX_25_32, _Buffer[i].Bytes[1]);
				}
				break;
			}
			case 33: {
				...
				*/
				if (notifySusiTriggerPulse) {
					notifySusiTriggerPulse(_Buffer[i].Bytes[1]);
				}
				break;
			}
			case 35: {
				...
				*/
				if (notifySusiMotorCurrent) {
					notifySusiMotorCurrent(ConvertTwosComplementByteToInteger(_Buffer[i].Bytes[1]));
				}
				break;
			}
			case 36: {	/* DEPRECATO DALLA REVISIONE 6 / 12 / 2020 */
				...
				*/
				if (notifySusiRealSpeed) {
					if (bitRead(_Buffer[i].Bytes[1], 7) == 1) {
						notifySusiRealSpeed(_Buffer[i].Bytes[1] - 128, SUSI_DIR_FWD);
					}
					else {
						notifySusiRealSpeed(_Buffer[i].Bytes[1] - 128, SUSI_DIR_REV);
					}
				}
				break;
			}
			case 37: {	/* DEPRECATO DALLA REVISIONE 6 / 12 / 2020 */
				...
				*/
				if (notifySusiRequestSpeed) {
					if (bitRead(_Buffer[i].Bytes[1], 7) == 1) {
						notifySusiRequestSpeed(_Buffer[i].Bytes[1] - 128, SUSI_DIR_FWD);
					}
					else {
						notifySusiRequestSpeed(_Buffer[i].Bytes[1] - 128, SUSI_DIR_REV);
					}
				}
				break;
			}
			case 38: {
				...
				*/
				...
				if (notifySusiMotorLoad) {
					notifySusiMotorLoad(ConvertTwosComplementByteToInteger(_Buffer[i].Bytes[1]));
				}
				break;
			}
			case 80: {
			...
			case 40: {
				...
				*/
				if (notifySusiAnalogFunction) {
					notifySusiAnalogFunction(SUSI_AN_FN_0_7, _Buffer[i].Bytes[1]);
				}
				break;
			}
			case 41: {
				...
				if (notifySusiAnalogFunction) {
					notifySusiAnalogFunction(SUSI_AN_FN_8_15, _Buffer[i].Bytes[1]);
				}
				break;
			}
			case 42: {
				...
				if (notifySusiAnalogFunction) {
					notifySusiAnalogFunction(SUSI_AN_FN_16_23, _Buffer[i].Bytes[1]);
				}
				break;
			}
			case 43: {
				...
				if (notifySusiAnalogFunction) {
					notifySusiAnalogFunction(SUSI_AN_FN_24_31, _Buffer[i].Bytes[1]);
				}
				break;
			}
			case 44: {
				...
				if (notifySusiAnalogFunction) {
					notifySusiAnalogFunction(SUSI_AN_FN_32_39, _Buffer[i].Bytes[1]);
				}
				break;
			}
			case 45: {
				...
				if (notifySusiAnalogFunction) {
					notifySusiAnalogFunction(SUSI_AN_FN_40_47, _Buffer[i].Bytes[1]);
				}
				break;
			}
			case 46: {
				...
				if (notifySusiAnalogFunction) {
					notifySusiAnalogFunction(SUSI_AN_FN_48_55, _Buffer[i].Bytes[1]);
				}
				break;
			}
			case 47: {
				...
				if (notifySusiAnalogFunction) {
					notifySusiAnalogFunction(SUSI_AN_FN_56_63, _Buffer[i].Bytes[1]);
				}
				break;
			}
			case 48: {
				...
				*/
				if (notifySusiAnalogDirectCommand) {
					notifySusiAnalogDirectCommand(1, _Buffer[i].Bytes[1]);
				}
				break;
			}
			case 49: {
				...
				*/
				if (notifySusiAnalogDirectCommand) {
					notifySusiAnalogDirectCommand(2, _Buffer[i].Bytes[1]);
				}
				break;
			}
			case 0: {
				...
				*/
				if (notifySusiNoOperation) {
					notifySusiNoOperation(_Buffer[i].Bytes[1]);
				}
				break;
			}
			case 94: {	//&& 95
				...
				*/
				...
				if (_Buffer[i + 1].Bytes[0] == 95) {				//i byte di comando devono susseguirsi
					if (notifySusiMasterAddress) {					// Controllo se e' presente il metodo per gestire il comando
						static uint16_t MasterAddress;

						MasterAddress = _Buffer[i + 1].Bytes[1];	//memorizzo i bit "piu' significativ"
						MasterAddress = MasterAddress << 8;			//sposto i bit 7 posti a 'sinistra'
						MasterAddress |= _Buffer[i].Bytes[1];	//aggiungo i 7 bit "meno significativi"
			...
			case 108: {
				...
				*/
				...
				if (notifySusiControllModule) {
					notifySusiControllModule(_Buffer[i].Bytes[1]);
				}
				break;
			}
			default: {}
